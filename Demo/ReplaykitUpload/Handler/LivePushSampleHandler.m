...
static LivePushSampleHandler *s_delegate;   // retain delegate
...
@interface ReplyKitEXTInternelLocalNotificaionPusher : NSObject

+ (void)sendLocalNotificationToHostAppWithTitle:(NSString*)title msg:(NSString*)msg userInfo:(NSDictionary*)userInfo;

@end

@interface LivePushSampleHandler() <TXLivePushListener, TXLiveBaseDelegate> {
...
@implementation LivePushSampleHandler {
...
//    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:[NSString stringWithFormat:@"推流地址:%@", s_rtmpUrl] userInfo:nil];
    [self start];
}
...
{
    [self stop];
    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"推流已停止" userInfo:nil];
}
...
- (void)handleReplayKit2RotateChangeNotification:(NSNotification*)noti
{
    ...
    if (!rotate) {
        [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"切换失败" userInfo:nil];
        return;
    }
...
//    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:s_landScape?@"切为横屏":@"切为竖屏" userInfo:nil];
...
//    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:s_resolution userInfo:nil];
    //test pausePush
...
//    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"推流启动" userInfo:nil];
}
...
- (void)broadcastStartedWithSetupInfo:(NSDictionary<NSString *,NSObject *> *)setupInfo {
    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"录屏已开始，请从这里点击回到Demo->直播->录屏推流->设置推流URL与横竖屏和清晰度" userInfo:@{kReplayKit2UploadingKey: kReplayKit2Uploading}];
}
...
- (void)broadcastPaused {
    ...
    [self pause];
    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"录屏已暂停" userInfo:nil];
}
...
- (void)broadcastResumed {
    ...
    [self resume];
    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"录屏已恢复" userInfo:nil];
}

- (void)broadcastFinished {
    // User has requested to finish the broadcast.
    NSLog(@"broadcastFinished");
    [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"录屏已结束" userInfo:@{kReplayKit2UploadingKey: kReplayKit2Stop}];
    [self stop];
}
...
-(void) onPushEvent:(int)EvtID withParam:(NSDictionary*)param {
    ...
    if (EvtID == PUSH_ERR_NET_DISCONNECT) {
        [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"推流失败!请换个姿势再来一次" userInfo:nil];
    }  else if (EvtID == PUSH_EVT_PUSH_BEGIN) {
        [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"连接成功！开始推流" userInfo:nil];
    } else if (EvtID == PUSH_WARNING_NET_BUSY) {
        [ReplyKitEXTInternelLocalNotificaionPusher sendLocalNotificationToHostAppWithTitle:@"腾讯云录屏推流" msg:@"网络上行带宽不足" userInfo:nil];
    }
}
...
@implementation ReplyKitEXTInternelLocalNotificaionPusher
