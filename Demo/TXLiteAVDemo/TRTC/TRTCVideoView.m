...
@property (nonatomic) TRTCRenderViewKey* viewKey;
...
+ (instancetype)newVideoViewWithType:(VideoViewType)type
                              key:(TRTCRenderViewKey * _Nullable)key
                            delegate:(id<TRTCVideoViewDelegate> _Nullable)delegate {
    TRTCVideoView* videoView = [TRTCVideoView new];
    videoView.type = type;
    videoView.viewKey = key;
    videoView.delegate = delegate;
    videoView.backgroundColor = UIColorFromRGB(0x262626);
...
- (void)showVideoCloseTip:(BOOL)show
{
    if (show) {
        if (!_tipBgView) {
            ...
            NSString* uidText = _viewKey.userId;
