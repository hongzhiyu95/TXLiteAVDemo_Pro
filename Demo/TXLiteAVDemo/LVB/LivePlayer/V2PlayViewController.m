...
@interface V2ToastTextView : UITextView
@property (nonatomic, strong) NSString *url;
@end

@interface V2PlayViewController() <
...
@implementation V2PlayViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [TXLiveBase setExternalDecoderFactory:[[TRTCHEVCDecoderFactory alloc] init]];
  ...
  //  TXLivePlayConfig* config = _player.config;
    ...
    // 允许接收消息
    [_player enableReceiveSeiMessage:YES payloadType:5];
...
- (void)initUI {
    ...
    _btnSwitchStream = [self createButton:@"h265" action:@selector(clickSwitchStream:)
                              center:CGPointMake(startSpace + ICON_SIZE / 2 + centerInterVal * 7, iconY) size:ICON_SIZE];
    // 菊花
    float width = 34;
    float height = 34;
...
- (void)clickHW:(UIButton *)btn {
  ...
  //  _player setD = !_player.enableHWAcceleration;
...
- (void)clickSwitchStream:(UIButton *)btn {
    if (![_player isPlaying]) {
        [self toastTip:@"请先启动播放!"];
        return;
    }
    if (![self.playUrl hasSuffix:@".flv"]) {
        [self toastTip:@"请在flv流下切换"];
        return;
    }
...
- (BOOL)startPlay {
  ...
  //  [_player setupVideoWidget:CGRectZero containView:_videoView insertIndex:0];
    ...
    if (sdkVersion > 10.7) {
        selString = @"startLivePlay:";
    
    }
    int ret = ((int (*) (id, SEL, NSString *) )objc_msgSend) (_player,
...
- (V2ToastTextView *)toastTip:(NSString*)toastInfo {
    CGRect frameRC = [[UIScreen mainScreen] bounds];
    frameRC.origin.y = frameRC.size.height - 110;
    frameRC.size.height -= 110;
    __block V2ToastTextView * toastView = [[V2ToastTextView alloc] init];
...
@implementation V2ToastTextView
