...
- (void)initCellInfos
{
    ...
    cellInfo.subCells = ({
        ...
        CellInfo *(^createTRTCItem)(NSString *title, TRTCAppScene scene, BOOL useCppWrapper) =
            ^(NSString *title, TRTCAppScene scene, BOOL useCppWrapper) {
                return [CellInfo cellInfoWithTitle:title
                    ...
                    [controller setUseCppWrapper:useCppWrapper];
                    return controller;
                }];
            };

        [subCells addObject:createTRTCItem(@"腾讯云视频通话", TRTCAppSceneVideoCall, NO)];
        [subCells addObject:createTRTCItem(@"视频互动直播", TRTCAppSceneLIVE, NO)];
#ifdef ENABLE_TRTC_CPP
        [subCells addObject:createTRTCItem(@"腾讯云视频通话 C++", TRTCAppSceneVideoCall, YES)];
        [subCells addObject:createTRTCItem(@"视频互动直播 C++", TRTCAppSceneLIVE, YES)];
#endif
        subCells;
    });
#endif
    ...
    cellInfo.subCells = ({
        NSMutableArray *subCells = [NSMutableArray new];
        CellInfo* cellInfo = [CellInfo cellInfoWithTitle:@"一房间"
                                     controllerClassName:@"ILiveRoomListViewController"];
        [subCells addObject:cellInfo];
        subCells;
    });
#endif
