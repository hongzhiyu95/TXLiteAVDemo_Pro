...
@interface TRTCVODViewController ()<
UITextFieldDelegate,
TXVodPlayListener,
#ifndef UGC_SMART
...
- (void)initUI {
    ...
    _playableProgress=[[UISlider alloc]initWithFrame:CGRectMake(70, hh-1, self.view.size.width-132, 30)];
    ...
    _playProgress=[[UISlider alloc]initWithFrame:CGRectMake(70, hh, self.view.size.width-140, 30)];
    ...
    _playoutVolumeSlider = [[UISlider alloc] initWithFrame:CGRectMake(70, hh-20, self.view.size.width-140, 30)];
    ...
    [_playoutVolumeSlider addTarget:self action:@selector(onSeek:) forControlEvents:(UIControlEventValueChanged)];
    [_playoutVolumeSlider addTarget:self action:@selector(onSeekBegin:) forControlEvents:(UIControlEventTouchDown)];
    [_playoutVolumeSlider addTarget:self action:@selector(onDrag:) forControlEvents:UIControlEventTouchDragInside];
    ...
    _btnChangeRate = [self createBottomBtnIndex:btn_index++ Icon:nil Action:@selector(clickChangeRate:) Gap:icon_gap Size:icon_size];
...
{
    UIButton* btn = [UIButton buttonWithType:UIButtonTypeCustom];
    btn.frame = CGRectMake((index+1)*gap + index*size, self.view.size.height - 2*(size + 10), size, size);
    [btn setImage:[UIImage imageNamed:icon] forState:UIControlStateNormal];
    [btn addTarget:self action:action forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:btn];
    return btn;
